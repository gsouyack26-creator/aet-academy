...
try:
    ...
    GLOSSARY_EXTRA = []
_seen = {g['term'].lower() for g in GLOSSARY}
...
DATA = (f"const MODS={json.dumps(MODS, ensure_ascii=False)};\nconst GLOSS={json.dumps(GLOSSARY, ensure_ascii=False)};\nconst TRACKS={json.dumps(A.TRACKS, ensure_ascii=False)};\nconst RANKS={json.dumps(A.RANKS, ensure_ascii=False)};\nconst XP={json.dumps(A.XP, ensure_ascii=False)};\nconst ACH={json.dumps(A.ACHIEVEMENTS, ensure_ascii=False)};\nconst FLASH={json.dumps(A.FLASHCARDS, ensure_ascii=False)};\nconst SIMS={json.dumps(A.SIMS, ensure_ascii=False)};\n")
DATA += f"const REF={json.dumps(REFERENCE, ensure_ascii=False)};\nconst REFCATS={json.dumps(REF_CATS, ensure_ascii=False)};\n"
PANELLINK_MAP={
...
DATA += f"const PANELLINK={json.dumps(PANELLINK_MAP, ensure_ascii=False)};\n"
LADDERLINK_MAP={
...
DATA += f"const LADDERLINK={json.dumps(LADDERLINK_MAP, ensure_ascii=False)};\n"
DATA += f"const LADDERLABS={json.dumps(LADDER_LABS, ensure_ascii=False)};\n"

VIEWS = "".join(f'<div class="view" id="v-{v}"></div>' for v in
...
<div id="sb">
  <div id="brand"><h1>&#9889; AET Academy</h1><div class="v">Automation Engineering Technology &bull; v13.78</div></div>
...
def build():
    ...
    with open(p, 'w', encoding='utf-8') as f:
        f.write(html)
    print(f"Built: {p} ({format(os.path.getsize(p), ',')} bytes)")
    print(f"  modules={len(MODS)} glossary={len(GLOSSARY)} tracks={len(A.TRACKS)} achievements={len(A.ACHIEVEMENTS)} flashcards(+gloss)={len(A.FLASHCARDS)+len(GLOSSARY)} sims={len(A.SIMS)} ladderlabs={len(LADDER_LABS)}")
