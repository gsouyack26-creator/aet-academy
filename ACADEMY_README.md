...
## v13.77 (2026-08-04) - Content Push Wave 8 + SR 3-Pass (0 thin sections, 0 short explains <90c)
Comprehensive content push across all 22 modules: Wave 8 expanded 9 thin sections (M1-M5) and 8 thin sections (M7-M12) from <215 chars to 500-750 chars each. Unicode normalization pass (230 replacements across 3 data files). SR Pass 1 audit identified 17 thin sections and 81 short explains remaining. SR Pass 2 fixed 22 explains. SR Pass 3 expanded thin sections: M2 Addressing, M2 IEC 61131-3 Languages, M3 Program Organization, M5 AC Induction Motors, M6 Troubleshooting, M7 Alarms, M8 Troubleshooting, M9 Robot Types, M9 Safety, M10 Feedback Control, M10 Tuning, M11 ISO 13849-1, M11 LOTO & Arc Flash, M12 Integration Mindset, M1 Motor Control Circuits, M13 UDTs, M15 Oscilloscope Basics. Final push: 58 quiz explains expanded across quiz2_data.py (27), quiz4_data.py (23), quiz5_data.py (1), and modules_data*.py (7). Result: 0 thin sections (<280c), 0 short explains (<90c). HTML grew from 3,306,529 bytes (v13.76) to 3,390,657 bytes (+84 KB content). Audit: sections=1140 quiz=1299 ERRORS=0. uvx ruff check = All checks passed.

## v13.76 (2026-07-28) - Build-script lint hygiene (deferred nits cleared)
Cleared the 22 non-auto-fixable ruff findings that v13.75 deferred - all in the two build scripts, no course content touched. build_academy.py: 10 blind `except Exception:` import guards narrowed to `except ImportError:` (correct intent - each wraps an optional `from X import Y` with a fallback assignment; all data modules are present so behavior is unchanged); the glossary2 merge block restructured so only the import is guarded, fixing a set()->set-comprehension nit (C401) and a try/except/pass (S110) at the same time; 10 printf-style `%`-format strings converted to f-strings (UP031, then UP032 for the collapsed multi-arg ones). reference_data.py: 2 `%`-format table-cell builders converted to f-strings. Result: `uvx ruff check .` = All checks passed (0 findings). Rebuilt HTML is byte-identical to v13.75 (3,306,529 bytes) and the build banner is unchanged (modules=22 glossary=131 flashcards+gloss=187 sims=11 ladderlabs=236). py_compile clean, audit ERRORS=0 (sections=1140 quiz=1299). Ship zip v13.76 (5 members, testzip None).

## v13.75 (2026-07-22) - Content fact-check QA sweep
Systematic accuracy review of the quiz bank. Extracted 378 fact-bearing questions (those with port numbers, standards refs, or numeric/unit claims) and hand-verified ~280 of them across 9 technical modules (M1 electrical, M2 PLC, M4 sensors, M5 VFD, M7 HMI, M8 networks, M10 PID, M11 safety, M14 IIoT, M17 panel design). Accuracy was ~99%. TWO genuine errors found and fixed:
1. M8 (Networks): a PowerFlex 755 drive question claimed "fault code 33 = ground fault." Verified against Rockwell sources that F33 is "Auto Restart Tries Exhausted"; the ground-fault code is F13. Corrected the stem and explanation to F13 (25% output-to-earth current threshold); the megger procedure in the answer was already correct.
2. M4 (Sensors): a 16-bit ADC resolution question had a broken answer key - the explanation computed 0.00061 mA but the marked option read 0.00024 mA and the explanation text was garbled. Corrected the option to 0.00061 mA and rewrote the explanation cleanly.
Also applied 16 safe ruff auto-fixes (11 UP009 unnecessary utf-8 declarations, 4 import-sorts, 1 FURB122) that a newer ruff surfaced across build scripts; 22 non-auto-fixable style/robustness nits (blind-except, printf-formatting) remain deferred as they touch runtime logic. Audit ERRORS=0, edited files ruff-clean, py_compile clean. Answer distribution stays balanced [327,325,325,322]. Build modules=22 glossary=131 flashcards+gloss=187 sims=11 ladderlabs=236. Ship zip v13.75 (5 members, testzip None).
...
- **v13.71** (2026-07-22) - Curriculum doubling Wave 5 (second deepening pass): added a second +6 sections / +9 quiz pass to ALL 22 modules. Content now 835 sections / 1013 quiz (from 703/815). New second-pass topics span HMI tag databases/ISA-101/trending/reports, industrial NAT/SNMP/PoE/cable certification, robot mounting/programming/EOAT/maintenance, level & temperature loop strategy/step-test ID/loop docs, E-stop categories/two-hand control/light-curtain distance/risk assessment, project estimating/UL 508A panels/closeout/business case, GSV-SSV/ALMD-ALMA/motion instructions/recipe UDTs, Unified Namespace/REST-webhooks/OEE/time-series DBs, sensor & encoder & VFD-fault & UPS & ground-fault diagnostics, field balancing/baseline signatures/backlog management/lubricant compatibility/failure coding, 24VDC redundancy/marshalling/condensation control/motor branch circuits/panel QA, home labs/case studies/business acumen/leadership transition, ST loops/ladder specials/FBD execution order/debugging by language, conveyor rollers-CEMA/guarding/product tracking/drum motors/PM routes, AMR localization/payload/wireless/floor infrastructure/diagnostics/fleet reliability.
